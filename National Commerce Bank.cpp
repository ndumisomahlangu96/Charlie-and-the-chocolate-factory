...
void showVector(const vector<int>);

int main()
{
    const int NUM_DIGITS = 7;   // Number of digits in a PIN
    vector<int> pin1 {2, 4, 1, 8, 7, 9, 0}; // Base set of values.
    vector<int> pin2 {2, 4, 6, 8, 7, 9, 0}; // Only 1 element is differnt from pin1.
    vector<int> pin3 {1, 2, 3, 4, 5, 6, 7}; // All elements are different from pin1.

    cout << "\t\tWelcome to National Commerce Bank.cpp\n\n";

    cout << "This program is a driver that tests a function comparing the\n"
...
//***************************************************************
// The following function accepts one int vector. The vector    *
// is then displayed to the user.                               *
//***************************************************************

void showVector(const vector<int> digits)
{
    for (int val : digits)
    {
        cout << val << " ";
